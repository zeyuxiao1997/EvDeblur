...
class ImageEventRestorationModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['datasets']['train'].get('use_mask'):
            ...
            preds = self.net_g(frame = self.lq, event = self.voxel)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                #     pred = self.net_g(frame = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :])  # mini batch all in 

                # elif self.opt['datasets']['val'].get('return_ren'):
                #     # pred = self.net_g(x = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :], ren = self.ren[i:j,:])
                #     pred = self.net_g(frame = self.lq[i:j, :, :, :], event = self.voxel[i:j, :, :, :])
